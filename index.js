...
async function run() {
  try {
    const usersCollection = client.db("focus").collection("users");
    const classesCollection = client.db("focus").collection("classes");
    const menuCollection = client.db("focus").collection("menu");
    const instaCollection = client.db("focus").collection("insta");
    ...
    app.patch("/users/insta/:id", async (req, res) => {
      const id = req.params.id;
      console.log(id);
      const filter = { _id: new ObjectId(id) };
      const updateDoc = {
    ...
    app.get("/instructor", async (req, res) => {
      const result = await instaCollection.find().toArray();
      res.send(result);
    });

    // cart collection apis get
    ...
    app.post("/carts", async (req, res) => {
      ...
      if (existingItem) {
        res.status(409).send("Item with the same _id already exists.");
      } else {
